...
//方法：把path和参数拼接起来，把字符串中的中文转换为 百分号形式。有的服务器不接收中文编码
...
/*习惯
 ...
 假设for循环 循环1000次，params.allKeys实际上调用哪个的[params allKeys],会调用allKeys1000次，oc语言特性是runtime，实际上我们调用一个方法，底层操作是，在一个方法列表中搜索你调用的方法所在的地址，然后调用完毕之后把这个方法名转移到常用方法列表。所以如果再次执行某个方法救治常用方法列表中搜索调用，效率更高。
*/
